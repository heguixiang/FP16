...
typedef enum tagFP16CompareType{
    EQUAL=0,
    NOT_EQUAL,
    GREATER_THAN,
    GREATER_EQUAL,
...
int Compare(int x, int y, fp16CompareType type){
...
int FCompare(uint32_t ux, uint32_t uy, fp16CompareType type){
    int result = 0;
    PREPARE_TWO_FP32_PARA
        switch (type){
        case EQUAL:             result = (x == y); break;
        case NOT_EQUAL:         result = (x != y); break;
        case GREATER_THAN:      result = (x > y);  break;
        case GREATER_EQUAL:     result = (x >= y); break;
        case LESS_THAN:         result = (x < y);  break;
        case LESS_EQUAL:        result = (x <= y); break;
    }
    return result;
}
...
uint32_t FMla(uint32_t x, uint32_t y, uint32_t z){
    float f1, f2, f3, fret;
    uint32_t result;
    f1 = *((float *)&x);
    f2 = *((float *)&y);
    f3 = *((float *)&z);
    fret = std::fma(f1, f2, f3);
    result = *((uint32_t *)&fret);
    return result;
}
...
int MultAddFP16(int x[], int y[], int z)
{
    ...
    u3 = *((uint16_t *)&z);
    fp.val = u3;
    fpRet = hf_mma(fp1, fp2, fp, MATRIX_LENGTH);
    ur = fpRet.val;
    result = ur;
    return result;
}
...
uint32_t MultAddFP32(int x[], int y[], float z)
{
    ...
    fRet = hf_mma(fp1, fp2, z, MATRIX_LENGTH);
...
uint32_t FAdd(uint32_t ux, uint32_t uy) {
    PREPARE_TWO_FP32_PARA
    float fRet = x + y;
    ret= *((uint32_t *)&fRet);
    return ret;
}
uint32_t FSub(uint32_t ux, uint32_t uy) {
    PREPARE_TWO_FP32_PARA
    float fRet = x - y;
    ret = *((uint32_t *)&fRet);
    return ret;
}
uint32_t FMul(uint32_t ux, uint32_t uy) {
    PREPARE_TWO_FP32_PARA
    float fRet = x * y;
    ret = *((uint32_t *)&fRet);
    return ret;
}
uint32_t FDiv(uint32_t ux, uint32_t uy) {
    PREPARE_TWO_FP32_PARA
    float fRet = x / y;
    ret = *((uint32_t *)&fRet);
    return ret;
...
uint32_t FloatMath(uint32_t ux, FPMathMethodType mathType){
    float x = *((float*)&ux);
    float fRet;
    switch (mathType)
    {
        case MATH_RCP: fRet = 1.0f/x; break;
        case MATH_SQRT: fRet = std::sqrt(x); break;
        case MATH_RSQRT: fRet = 1.0f / std::sqrt(x); break;
        case MATH_ABS: fRet = std::abs(x); break;
        case MATH_EXP: fRet = std::exp(x); break;
        case MATH_LN: fRet = std::log(x); break;
        case MATH_LOG2: fRet = std::log2(x); break;
        case MATH_LOG10: fRet = std::log10(x); break;
        case MATH_POW2: fRet = std::pow(2, x); break;
        case MATH_POW10: fRet = std::pow(10, x); break;
        case MATH_SIN: fRet = std::sin(x); break;
        case MATH_COS: fRet = std::cos(x); break;
        default:
            fRet = 0;
            break;
    }
    uint32_t ret = *((uint32_t *)&fRet);
    return ret;
}
...
PyObject* WrappDeq(PyObject* self, PyObject* args)
{
    ...
    return Py_BuildValue("i", Deq(x, y));
}
...
/***compile command：****************************************************************************************************/
/***g++ -std=c++11 -fPIC -shared fp16_t.cc fp16_math.cc fp16_unit.cc fpy.cpp -I/usr/include/python2.7 -o fpy.so**********/
/***g++ -std=c++11 -fPIC -shared fp16_t.cc fp16_math.cc fp16_unit.cc fpy.cpp -I/usr/include/python3.5 -o fpy.so**********/
/************************************************************************************************************************/
